...
public class CommunityPostMockData {
    ...
    private List<CommunityPost> createPosts() {
        ...
        for (int postNumber = 1; postNumber <= 34; postNumber++) {
            CommunityPost post = CommunityPost.create(
                    ...
                    createContentJson(postNumber),
                    createContentText(postNumber),
                    "",
                    createCategory(postNumber),
    ...
    private Long createPostId(int postNumber) {
        // 게시글 번호 생성
        return (long) postNumber;
    }
    ...
    private String createCategory(int postNumber) {
        // 구분 생성
        if (postNumber % 10 == 0) {
            return "notice";
        }
    ...
    private String createContentJson(int postNumber) {
        // 본문 JSON 생성
        return """
                {"type":"doc","content":[{"type":"paragraph","content":[{"type":"text","text":"%s"}]}]}
                """.formatted(escapeJson(createContentText(postNumber)));
    }
