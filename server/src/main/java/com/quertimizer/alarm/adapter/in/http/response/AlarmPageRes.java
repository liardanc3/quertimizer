...
public class AlarmPageRes {
    ...
    public static AlarmPageRes from(AlarmPageOutput result) {
        return new AlarmPageRes(
                result.getCurrentPage(), result.getPageSize(), result.getTotalCount(),
                result.getTotalPages(), result.getUnreadCount(),
                result.getAlarms().stream().map(AlarmItemRes::from).toList()
        );
    }
}
