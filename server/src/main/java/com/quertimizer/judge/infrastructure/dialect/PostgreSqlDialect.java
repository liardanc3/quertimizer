...
public class PostgreSqlDialect implements JudgeDialect {
    ...
    public String selectCountSql(String sql) {
        return "SELECT COUNT(*) FROM (" + sql + ") sql_judge_result_count";
    }

    @Override
    public String selectPageSql(String sql) {
        return "SELECT * FROM (" + sql + ") sql_judge_result_page LIMIT ? OFFSET ?";
    }
}
