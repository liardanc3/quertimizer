...
import com.quertimizer.global.config.JudgeProperties;
...
public class ProcessDockerContainerAdapter implements DockerContainerPort {
    ...
    public List<DockerContainerOutput> findJudgeContainers() {
        // judge 설정의 runner container 이름 기준 docker stats 조회
        Set<String> containerNames = new LinkedHashSet<>();
        for (JudgeProperties.DatabaseProperties database : judgeProperties.getDatabases()) {
            if (database.getRunnerContainer() != null && !database.getRunnerContainer().isBlank()) {
                containerNames.add(database.getRunnerContainer().trim());
            }
        }
