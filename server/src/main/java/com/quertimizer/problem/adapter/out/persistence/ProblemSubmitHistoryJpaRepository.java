...
public interface ProblemSubmitHistoryJpaRepository extends JpaRepository<ProblemSubmitHistoryJpaEntity, Long> {

    @Query(value = """
            select history.handle as handle, count(history) as submitCount
            from ProblemSubmitHistoryJpaEntity history
    ...
    List<ProblemSubmitHistoryJpaEntity> findAllByHandleOrderBySubmittedAtDesc(String handle);
