...
import com.quertimizer.service.AdminAnomalyDetectionService;
...
public class AdminAnomalyDetectionController {
    ...
    @PostMapping("/admin/anomaly-accounts/users/{userId}/block")
    public ResponseEntity<Void> blockUser(@PathVariable String userId) {

        accountRestrictionService.blockUser(userId);
        return ResponseEntity.noContent().build();
    }

    @DeleteMapping("/admin/anomaly-accounts/users/{userId}/block")
    public ResponseEntity<Void> unblockUser(@PathVariable String userId) {

        accountRestrictionService.unblockUser(userId);
        return ResponseEntity.noContent().build();
    }
