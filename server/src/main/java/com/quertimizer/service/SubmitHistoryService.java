...
import com.quertimizer.endpoint.api.dto.response.SubmitHistoryListItemRes;
import com.quertimizer.endpoint.api.dto.response.SubmitHistoryPageRes;
...
public class SubmitHistoryService {
    ...
    public SubmitHistoryPageRes getSubmitHistories(int requestedPage,
                                                   String submitId,
                                                   String query,
                                                   String dbms,
                                                   String problemId,
        ...
        List<SubmitHistoryListItemRes> filteredHistories = histories.stream()
                .filter(history -> matchesSubmitId(history, submitId))
                .filter(history -> matchesUserId(history, query))
    ...
    private boolean matchesUserId(ProblemSubmitHistory history, String query) {
        if (query == null || query.isBlank()) {
            return true;
        }

        return history.getUserId().toLowerCase(Locale.ROOT).contains(query.trim().toLowerCase(Locale.ROOT));
    }
