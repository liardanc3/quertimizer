...
public class CommunityCommentMockData {
    ...
    public void seed() {
        for (CommunityPost post : communityPostRepository.findAll().stream()
            ...
            CommunityComment rootComment = communityCommentRepository.save(CommunityComment.create(
                    post.getPostId(),
                    resolveRootCommentUserId(postNumber),
                    null,
            ...
            if (postNumber <= 10) {
                communityCommentRepository.save(CommunityComment.create(
                        post.getPostId(),
                        resolveReplyCommentUserId(postNumber),
    ...
    private String resolveRootCommentUserId(int postNumber) {
        if (postNumber <= 10) {
    ...
    private String resolveReplyCommentUserId(int postNumber) {
