...
(10, 'community-seed-0010', 10, 'Oracle에서 FETCH FIRST가 아직 낯섭니다', 'ROWNUM에 익숙한 입장에서는 FETCH FIRST 구문이 더 선언적으로 느껴졌다.', '정렬 후 상위 N건을 가져오는 흐름을 읽기 쉽게 표현할 수 있는 점이 좋았다.', 'DBMS마다 익숙한 문법은 달라도 결국 의도를 분명히 적는 게 우선이다.', TIMESTAMP '2026-03-06 16:05:00', ARRAY['Oracle', '질문', '페이징']),
...
(28, 'community-seed-0028', 28, 'Oracle 힌트는 언제부터 유지보수 비용이 커지는지', '힌트가 즉시 효과를 줄 때도 있지만 쿼리와 통계가 바뀌면 유지 비용이 빠르게 커진다.', '문제 해결 후에도 왜 힌트가 필요했는지 기록하지 않으면 나중에 더 위험했다.', '힌트는 최후 수단에 가깝게 쓰는 편이 결국 안정적이었다.', TIMESTAMP '2026-03-14 08:45:00', ARRAY['Oracle', '힌트', '상급자']),
